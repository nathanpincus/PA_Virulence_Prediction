# # Predicting virulence (High/Low) of 25 PASP isolates using BLSF_noCF final GSCV-tuned RF model - trained on core variant SNVs feature set
...
# Load in GSCV object
with open("/path/to/BLSF_CG_OHE_RF_GridSearch.pkl", "rb") as f:
    GSCV = pickle.load(f)
print("GSCV Object:")
print(GSCV)
...
# ### Load in PASP features (core variant SNVs positons as defined in the training set) and convert to a dataframe
# Read in fasta file
fasta = open("/path/to/PASPcoreSNVs.fasta").readlines()
...
Scores.to_csv("PASP_VirulencePredictions_BLSF_RF_coreSNVs.csv", index=False)

# Plot ROC curve
print("Plot AUC curve:")
file_auc = "PASP_VirulencePredictions_BLSF_RF_coreSNVs_AUC.pdf"
...
PASP["pred"] = ypred
Comp = PASP[["rank","pred"]]
Comp.to_csv("PASP_virrank_vs_prediction_coreSNVs.csv")
