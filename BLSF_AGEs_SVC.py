# # Using SVC to predict virulence based on Unique AGEs in the BLSF_noCF dataset
...
param_grid = {'C': [10 ** x for x in range(-4,5)], 'gamma': [10 ** x for x in range(-8,4)]}
...
SVC_model = SVC(kernel='rbf', random_state=s, probability=True)
SVC_GridSearch = GridSearchCV(SVC_model, param_grid, cv=stratkf, iid=False, n_jobs=-1)
...
# Determing CV_scores
CV_scores = cross_validate(SVC_GridSearch, X, y, scoring=scorer, cv=stratkf)
...
CV_df.to_csv('BLSF_AGEs_SVC_GSCV_NestedCVResults.csv', index=False)
...
SVC_model = SVC(kernel='rbf', random_state=s, probability=True)
SVC_GridSearch_final = GridSearchCV(SVC_model, param_grid, cv=stratkf, iid=False, n_jobs=-1)

# Fit the data to the model
SVC_GridSearch_final.fit(X, y)

#Print model info
print("Final model SVC grid search results:", flush=True)
print(SVC_GridSearch_final, flush=True)

#Print internal best score
#Note: this should not be considered a representation of how the model performs on new data
print("Best internal accuracy: " + str(SVC_GridSearch_final.best_score_), flush=True)

#Print best parameters
print("Best parameters:", flush=True)
print(SVC_GridSearch_final.best_params_, flush=True)

#Print complete CV results
print("Complete CV results:", flush=True)
print(SVC_GridSearch_final.cv_results_, flush=True)

#Extract out best Estimator
SVC_final = SVC_GridSearch_final.best_estimator_
print("Extracted model from GSCV", flush=True)
print(SVC_final)

#Pickle GCSV, best estimator, X, and y
with open('BLSF_AGEs_SVC_GSCV_GridSearch.pkl', 'wb') as f:
    pickle.dump(SVC_GridSearch_final, f)
with open('BLSF_AGEs_SVC_GSCV_Model.pkl', 'wb') as f:
    pickle.dump(SVC_final, f)
with open('BLSF_AGEs_SVC_GSCV_Features.pkl', 'wb') as f:
    pickle.dump(X, f)
with open('BLSF_AGEs_SVC_GSCV_labels.pkl', 'wb') as f:
    pickle.dump(y, f)
...
file_lc = "BLSF_AGEs_SVC_LearningCurve_95CI.pdf"
plot_learning_curves_function_95CI.plot_learning_curve(
    estimator=SVC_GridSearch, X=X, y=y, scoring="accuracy", cv=stratkf, n_jobs=1,
