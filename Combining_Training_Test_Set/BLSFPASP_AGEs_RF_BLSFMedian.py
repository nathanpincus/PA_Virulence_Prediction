# # Using Random Forest to predict virulence based on Unique AGEs in all 140 tested strains - BLSF median cutoff
...
UG = pd.read_csv("/path/to/BLSFPASP_tested.uniquegroups.gte200.csv", index_col="genome")

# Import in labels.
# Genome name as index column
BLSF_vir = pd.read_csv("/path/to/LD50_estimates_training.csv", index_col=0)
vir = pd.read_csv("/path/to/LD50_estimates_all.csv", index_col=0)
vir.loc[vir.rounded < BLSF_vir.rounded.median(), "rank_BLSF_median"] = 1
vir.loc[vir.rounded >= BLSF_vir.rounded.median(), "rank_BLSF_median"] = 0
vir.loc[vir.rounded < vir.rounded.median(), "rank_all_median"] = 1
vir.loc[vir.rounded >= vir.rounded.median(), "rank_all_median"] = 0

# Combine features and labels for easy splitting into X and y
# Set all column datatypes to float for consistency
UG["rank"] = vir["rank_BLSF_median"]
...
CV_df.to_csv('BLSFPASP_AGEs_RF_BLSFMedian_GSCV_NestedCVResults.csv', index=False)
...
with open('BLSFPASP_AGEs_RF_BLSFMedian_GSCV_GridSearch.pkl', 'wb') as f:
    pickle.dump(RF_GridSearch_final, f)
with open('BLSFPASP_AGEs_RF_BLSFMedian_GSCV_Model.pkl', 'wb') as f:
    pickle.dump(RF_final, f)
with open('BLSFPASP_AGEs_RF_BLSFMedian_GSCV_Features.pkl', 'wb') as f:
    pickle.dump(X, f)
with open('BLSFPASP_AGEs_RF_BLSFMedian_GSCV_labels.pkl', 'wb') as f:
    pickle.dump(y, f)
...
file_lc = "BLSFPASP_AGEs_RF_BLSFMedian_LearningCurve_95CI.pdf"
